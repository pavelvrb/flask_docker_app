...
SELECT array_agg(movieId) as user_views FROM ratings WHERE userID=1;

-- таблица user_movies_agg, в которую сохраните результат предыдущего запроса
DROP TABLE IF EXISTS user_movies_agg;
SELECT array_agg(movieId) INTO user_movies_agg FROM ratings WHERE userID=1;
...
-- Подсказка: нужно заджойнить user_movies_agg и common_user_views и применить вашу функцию к соответствующим полям.
